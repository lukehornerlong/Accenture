...
type RestOf<T extends IPost | NewPost> = Omit<T, 'timestamp'> & {
...
export class PostService {
  ...
  updatePost(post: IPost): Observable<IPost> {
    const url = `${this.resourceUrl}/${post.id}`;
  ...
  protected convertDateFromClient<T extends IPost | NewPost | PartialUpdatePost>(post: T): RestOf<T> {
    return {
      ...post,
      timestamp: post.timestamp?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restPost: RestPost): IPost {
    return {
      ...restPost,
      timestamp: restPost.timestamp ? dayjs(restPost.timestamp) : undefined,
    };
  }
